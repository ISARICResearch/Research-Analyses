...
These are the figures and tables for the Mpox study in South Kivu, DRC. The attached code is to process the raw data (saving a clean CSV and a data dictionary) and to perform the data analyses (saving data and metadata files used by ISARIC VERTEX tool, commit e4ad182). 
...
After this, you should be able to run an ISARIC VERTEX dashboard (at the time of writing, the commit is e4ad182), which will take the files saved and create the figures.
